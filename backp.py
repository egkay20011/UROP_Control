def propagate(initial_conditions,force,weight_array):
    ...
    cost_arr = np.array([[sigmoid(force),0]])
    ...
    for i in range(np.shape(weight_array[2])[1]):
        for y in range (np.shape(weight_array[2])[0]):
            ...
            #grad_final_array[y][i] = second_layer[0][y]*2*(result[0][i]-cost_arr[0][i])*sigmoid(z)*(1-sigmoid(z))
            grad_final_array[y][i] = second_layer[0][y]*2*(sigmoid(z)-cost_arr[0][i])*sigmoid(z)*(1-sigmoid(z))
    ...
    for i in range(np.shape(weight_array[1])[1]): #for every neuron
        for y in range (np.shape(weight_array[1])[0]):
            ...
            for ki in range(np.shape(weight_array[2])[1]):
                ...
                prog_func += weight_array[2][y][ki]*(1-sigmoid(w_3_c))*sigmoid(w_3_c)*2*(sigmoid(sigmoid(w_2_b)*weight_array[2][y][ki])-cost_arr[0][ki])*(1-sigmoid(w_2_b))*sigmoid(w_2_b)*first_layer[0][i]
        ...
        for y in range(np.shape(weight_array[0])[1]): #for each weight assigned to a first layer neuron
            ...
            for x in range(np.shape(weight_array[1])[1]): # for each weight leaving the receptacle neuron
                for k in range(np.shape(weight_array[2])[1]):#for each final layer connection
                    ...
                    prog_func += sigmoid(w_1_a)*(1-sigmoid(w_1_a))*initial_conditions[0][i] * sigmoid(w_2_b)*(1-sigmoid(w_2_b))*weight_array[1][y][x] *sigmoid(w_3_c)*(1-sigmoid(w_3_c))*weight_array[2][x][k]*2*(sigmoid(w_3_c)-cost_arr[0][k])
